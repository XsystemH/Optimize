...
# Usage: scripts/test_llvm_ir_all_reimu.bash <compiler> <codegen_dir> [builtin]
...
if [ ! -x ${CODEGEN_DIR}scripts/test_llvm_ir_reimu.bash ]; then
    echo "Error: ${CODEGEN_DIR}scripts/test_llvm_ir_reimu.bash does not exist or is not executable." >&2
    exit 1
...
judge_one_testcase() {
    ...
    mkdir -p $TESTDIR
    ${CODEGEN_DIR}scripts/test_llvm_ir_reimu.bash "$COMPILER" $1 $BUILTIN $TESTDIR > /dev/null 2>&1
    if [ $? -ne 0 ]; then
        FAILED_TESTCASE="$2"
